 ...
 */
export function useWheel(params) {
    const { orientation, direction, discretePaging, threshold, debounce, cooldown = 800, // 800ms cooldown - reduced double triggers
    onRequestIndexChange } = params;
    ...
    const onWheel = useCallback((evt) => {
        if (!discretePaging)
            return; // allow native scroll; index derived elsewhere
        evt.preventDefault();
        evt.stopPropagation();
        ...
        const currentDirection = adjusted > 0 ? 1 : -1;
        ...
        if (Math.abs(accumulated.current) >= threshold) {
            const delta = accumulated.current > 0 ? 1 : -1;
            // Reset everything
            accumulated.current = 0;
            gestureDirection.current = null;
            eventCount.current = 0;
